...
Agent.prototype.request = function request(options, callback) {
  ...
  // * HTTP/2 over plain TCP
    ...
    if(options.socket) {
      ...
      endpoint.socket.on('close', function () {
        if (key in self.endpoints) delete self.endpoints[key];
      });
    } else {
...
Agent.prototype.removeEndpoint = function(options) {
  options.host = options.hostname || options.host || 'localhost';
  options.port = options.port || 443;
  ...
  if (key in this.endpoints) delete this.endpoints[key];
}
